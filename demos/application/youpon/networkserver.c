		...
		if(is_can_add == 1 )
		{
			...
			if(endpointArray)
			{
				...
				for(j=0; j < SENSOR_CHANL_MAX; j++)
				{
					...
					//if((dev_class != 300  && 1 == alloc_flag) || (dev_class == 300 &&  1 == alloc_flag && value !=0 ))
					if( 1 == alloc_flag)
					{
			...
			{
			#ifdef USER_DEBUG_
				Network_log("getSensorListInfo() send data2server=%s \n", out);	
			#endif
...
static  int removeSensorDevice( char mac_str2[], char nwk_str2[] )
{
	int ret = -1;
	int i;
	uint8_t is_can_add;
...
static void zigbee_request_all_device_mac(uint16_t nwid)
{
	KEY_BUTTON_CTRL_PARAM_T  ctrlparam;
	memset(&ctrlparam, 0, sizeof(KEY_BUTTON_CTRL_PARAM_T));
...
OSStatus getSceneListInfo(uint32_t device_id)
{
	...
	#ifdef USER_DEBUG_
		//Network_log("sceneList send server: ");	
	#endif
		...
		for(j=0; j < DEVICE_INFO_LIST_MAX; j++)
		{
			...
			if(net_addr != 0 && net_addr != 0xFFFF)
			{
				...
				if(out)
				{
					...
					//sendMessageToServer(out, strlen(out), device_id);
					pushSend2ServerMsgProcess((uint8_t *)out, strlen(out), device_id);
...
OSStatus getDeviceListInfo(uint32_t device_id)
{
	...
	#ifdef USER_DEBUG_
		Network_log("DeviceList send server: ");	
	#endif
