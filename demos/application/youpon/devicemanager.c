...
extern int test_add_new_bind_device(uint32_t dev_type, uint16_t nwd_id, int32_t val, uint8_t dstp, uint8_t chnl_id);
...
void clear_all_device_recode_inflash()
{
	app_context_t* app_context = get_app_context();
...
void  print_device_infomation()
{
	...
	else
	{
		...
		for(i=0; i < DEVICE_INFO_LIST_MAX; i++)
		{
			...
			mico_rtos_lock_mutex(&app_context->userDataStore->device_info_mutex);
			#ifdef USER_DEBUG_
			device_mng("%d<==> id:%d  net_addr:0x%04x  dev_class:0x%08x  is_used:%d  dev_onoffline:%d   riu:%d",i, id,net_addr,dev_class,is_used,dev_onoffline,riu);
			device_mng("mac:%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5],mac[6],mac[7],mac[8],mac[9],mac[10],mac[11]);		
			#endif
			mico_rtos_unlock_mutex(&app_context->userDataStore->device_info_mutex);
...
{
	device_mng("zigbee_request_subdevice_state.\n");
	uint16_t dev_addr;
	uint8_t is_can_add;
	uint8_t riu;
	int i;
...
int rs485_update_device_state(uint16_t dev_addr, uint8_t endpoint, int32_t value)
{
	int ret = -1;
	int i;
	uint8_t is_can_add;
	...
	for(i=0; i < DEVICE_INFO_LIST_MAX; i++)
	{
		...
		device_mng("update state::::  endpoint=%d is_can_add=%d nwkaddr=%04x src=%04x  value=%d",endpoint, is_can_add, nwkaddr, dev_addr, value);
		if(is_can_add == 1 && nwkaddr == dev_addr)
		{
...
int device_logout_update_flag(uint16_t nwk_addr)
{
	...
	for(i=0; i < DEVICE_INFO_LIST_MAX; i++)
	{
		...
		if(is_can_add == 1 && nwkaddr == nwk_addr)
		{
			...
			memset(&(app_context->userDataStore->userData.deviceinfo.device_info[i]), 0, sizeof(device_info_list_t));
			mico_rtos_unlock_mutex(&app_context->userDataStore->device_info_mutex);
			device_mng("update device info:%d\n", i);
			ret =  user_data_store_update();
...
int device_light_one_report_attribute_update(ZB_DEV_JOINED_REQUEST_T *pDevInfo)
{
	...
	for(i=0; i < DEVICE_INFO_LIST_MAX; i++)
	{
		...
		if(is_can_add == 1 && nwkaddr == pDevInfo->src)
		{
			...
			if(pDevInfo->sep == 1)
			{
				app_context->userDataStore->userData.deviceinfo.device_info[i].chnl[0].endpoint_id = pDevInfo->sep;
			}
...
int device_light_two_report_attribute_update(ZB_DEV_JOINED_REQUEST_T *pDevInfo)
{
	...
	for(i=0; i < DEVICE_INFO_LIST_MAX; i++)
	{
		...
		if(is_can_add == 1 && nwkaddr == pDevInfo->src)
		{
			...
			if(pDevInfo->sep == 2)
			{
				app_context->userDataStore->userData.deviceinfo.device_info[i].chnl[1].endpoint_id = pDevInfo->sep;
			}
...
int device_light_three_report_attribute_update(ZB_DEV_JOINED_REQUEST_T *pDevInfo)
{
	...
	for(i=0; i < DEVICE_INFO_LIST_MAX; i++)
	{
		...
		if(is_can_add == 1 && nwkaddr == pDevInfo->src)
		{
			...
			if(pDevInfo->sep == 3)
			{
				app_context->userDataStore->userData.deviceinfo.device_info[i].chnl[2].endpoint_id = pDevInfo->sep;
			}
...
int zigbee_new_device_add_report(ZB_DEV_JOINED_REQUEST_T *pDevInfo)
{
	...
	for(i=0; i < DEVICE_INFO_LIST_MAX; i++)
	{
		...
		if(is_can_add == 1 && !memcmp(dev_mac, pDepStateInfo->mac, ZIGBEE_DEVICE_MAC_ADDRESS_LENGHT))
		{
			...
			mico_rtos_lock_mutex(&app_context->userDataStore->device_info_mutex);
			app_context->userDataStore->userData.deviceinfo.device_info[i].dev_onoffline = 1;
			app_context->userDataStore->userData.deviceinfo.device_info[i].net_addr = pDevInfo->src;
			mico_rtos_unlock_mutex(&app_context->userDataStore->device_info_mutex);
			device_mng("update device info i=%d \n", i);
			ret =  user_data_store_update();
			return ret;
		}
...
static int  rs485_add_integradedevice2list(uint16_t dev_addr, uint8_t devType[], uint8_t device_type_count, uint8_t rs485dev_mac[])
{
	...
	for(i=0; i < DEVICE_INFO_LIST_MAX; i++)
	{
		...
		if(is_can_add == 1 && !memcmp(dev_mac, rs485dev_mac, DEVICE_MAC_ADDRESS_LENGHT))
		{
			...
			mico_rtos_unlock_mutex(&app_context->userDataStore->device_info_mutex);
			new_add_device_index = i;
			device_mng("update device info i=%d,dev_addr:%04x \n", i,dev_addr);
			ret =  user_data_store_update();
			break;
		}
		else if((is_can_add == 1) && memcmp(dev_mac, rs485dev_mac, DEVICE_MAC_ADDRESS_LENGHT) && (dev_class == 299))// new ctr 485
		{
			...
			mico_rtos_unlock_mutex(&app_context->userDataStore->device_info_mutex);
			new_add_device_index = i;
			device_mng("delete device info i=%d \n", i);
			ret =  user_data_store_update();
			break;
		}
...
int rs485_new_device_joined_success_ack(RS485_DATA_FORMAT_T *pRs485Data)
{
	...
	device_mng("@@@@@@@@@@@@ rs485_dev_joined_request device_type_count=%d devicetype=%d \n", device_type_count, devicetype);
...
int save_scenelist_parame(uint16_t groupid, uint8_t sceneid)
{
	int i, j , bit=0, ret=0;
	...
	device_mng("save_scenelist_parame in, groupid=%d, sceneid=%d", groupid, sceneid);
...
int write_scene_info(uint16_t net_addr, uint8_t *value, int len)
{
	...
	//device_mng("net_addr:0x%04x\r\nvalue:",net_addr);
	//HexDump(value+4, len-4); // 4 is groupId, sceneId, statusCount.
