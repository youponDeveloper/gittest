...
int application_start( void )
{
	...
	err = mico_rtos_create_thread(NULL, MICO_APPLICATION_PRIORITY, "xlink domain_loop", xlink_domain_loop, 0x900,  (mico_thread_arg_t)app_context);
	require_noerr_string( err, exit, "ERROR: Unable to start the xlink_domain_loop" );

	err = mico_rtos_create_thread(NULL, MICO_DEFAULT_WORKER_PRIORITY, "xlink thread", xlink_function_thread, 0x1500,  (mico_thread_arg_t)app_context );
