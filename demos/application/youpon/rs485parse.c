...
extern int rs485_update_device_state(uint16_t dev_addr, uint8_t endpoint, int32_t value);
...
OSStatus  rs485_device_report_state(uint32_t  len, RS485_DATA_FORMAT_T *pRs485Data)
{
	...
	if( pRs485Data->dataheader.cmdID == USER_NUMBER_VAL_1)
	{
	...
	else if(pRs485Data->dataheader.cmdID == USER_NUMBER_VAL_3)
	{
...
OSStatus  rs485_msg_state_report(RS485_DATA_FORMAT_T *pRs485Data)
{
	...
	else if(pRs485Data->dataheader.cmdID == 4||pRs485Data->dataheader.cmdID == 0x0A)
	{		
	...
	rs485_update_device_state(dev_addr,  setPiont, setValue);
...
OSStatus  rs485_msg_zcl_zdo(cJSON *root)
{
	...
	unsigned short  nclu;
	uint32_t  data_len=0;
	unsigned char tmp[USER_NUMBER_VAL_128]={0};
	...
	if(objectValue1->valueint == 15)
		return err;

	if((nclu!= RS485_DEVICE_REPORT_STATE)||(nclu== RS485_DEVICE_REPORT_STATE && pileId == 0x0008)||(nclu== RS485_DEVICE_REPORT_STATE && pileId == 0x0402)
