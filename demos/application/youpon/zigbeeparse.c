...
static OSStatus  zigbee_msg_attribute_state_report(cJSON *root,  ZB_ZCL_ZDO_MSG_T *pZclZdoMsgInfo)
{
	...
	switch(device_type)
	{
		...
		{
			kitchen_panel_device_report_attribute_update(&msgInfo);		
		}
		break;
		case ZB_TYPE_SINGLE_LIGHT:
			device_light_one_report_attribute_update(&msgInfo);
		break;
		case ZB_TYPE_DOUBLE_LIGHT:
			device_light_two_report_attribute_update(&msgInfo);
		break;
		case ZB_TYPE_THREE_LIGHT:
			device_light_three_report_attribute_update(&msgInfo);
		break;
		case ZB_TYPE_SCENEPANEL:
			scenepanel_device_report_attribute_update(&msgInfo);
		break;
...
static OSStatus zigbee_parser_scene_cluster_msg(cJSON *root, ZB_ZCL_ZDO_MSG_T *pZclZdoMsgInfo)
{
	...
	{
		save_scenelist_parame(groupid, sceneid);
	}
...
int uart_zigbee_queue_init()
{
	...
	err = mico_rtos_init_queue(&uart_zigbee_msg_recv_queue, "zb_recv_queue", sizeof(int), 10);
...
int uart_zigbee_recv_start( app_context_t *app_context )
{
	...
	err = mico_rtos_create_thread(NULL, MICO_APPLICATION_PRIORITY, "user_zigbee_handler", 
	                            uart_zigbee_msg_recv, 0x1200, 0 );
	require_noerr_string( err, exit, "ERROR: Unable to start the user_zigbee_handler" );
	
	err = mico_rtos_create_thread(NULL, MICO_APPLICATION_PRIORITY, "popRecvFromFlogCloudMsgThread", 
	                            popRecvFromFlogCloudMsgThread, 0x1200, 0 );
	
	require_noerr_string( err, exit, "ERROR: Unable to start the popRecvFromFlogCloudMsgThread" );

	err = mico_rtos_create_thread(NULL, MICO_APPLICATION_PRIORITY, "rs485_uart_recv", 
					rs485_uart_data_recv, 0x2800, 0 );
	require_noerr_string( err, exit, "ERROR: Unable to start the rs485_uart_recv" );

	err = mico_rtos_create_thread(NULL, MICO_APPLICATION_PRIORITY, "request_DeviceState", 
	                request_DeviceState, 0x900, (uint32_t)app_context );
